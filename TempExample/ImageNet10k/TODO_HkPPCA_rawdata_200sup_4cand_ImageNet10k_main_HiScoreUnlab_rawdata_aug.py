...
def HkPPCAs(num_labeled_per_class, q_ini, q_train, lda, num_iters, dataset_name, self_learner_name, feature_address,
    ...
    for session_id in range(num_sessions):
        ...
        num_batches = 240 #120#150 #300  # 80
        ...
        ImageNet10kaug_addr='K:\\Project Data\\ImageNet10kAug\\'
        Aug_alltr_feature_address = [ImageNet10kaug_addr+'features_train_augmented2x_ImageNet10k_288_resnetx4_CLIP']

        # ## - subtask: integrate features into a large .pth feature file
        # ## train set is too large for memory, divide it into two parts.
        # train_aug_set_first500 = modules.combine_ImageNet10k_by_part(Aug_alltr_feature_address, n_features=640,first=True) # for resnet x4
        # train_aug_values=train_aug_set_first500.values
        # train_aug_labels=train_aug_set_first500.labels# list
        # # N_first500=len(train_aug_labels)
        # ## save data as .pth
        # addr_aug = ImageNet10kaug_addr+ \
        #            'Test_DataAug_AugmentedImageNet10k_80ratio_x2perimg_trainset_CLIP_eval_resnetx4_first5000.pth'
        # torch.save({'feature': train_aug_values.cpu().to(torch.float),'label':train_aug_labels}, addr_aug)
        #
        # train_aug_set_second500 = modules.combine_ImageNet10k_by_part(Aug_alltr_feature_address, n_features=640,first=False)  # for resnet x4
        # train_aug_values = train_aug_set_second500.values
        # train_aug_labels = train_aug_set_second500.labels  # list
        # # N_second500 = len(train_aug_labels)
        # ## save data as .pth
        # addr_aug = ImageNet10kaug_addr + \
        #            'Test_DataAug_AugmentedImageNet10k_80ratio_x2perimg_trainset_CLIP_eval_resnetx4_second5000.pth'
        # torch.save({'feature': train_aug_values.cpu().to(torch.float),'label':train_aug_labels}, addr_aug)
        # ## -/subtask
        ...
        # if cls_range[1]<501:
        #     addr_aug = checkpoint_folder_address + \
        #            'Test_DataAug_AugmentedImageNet10k_80ratio_x2perimg_trainset_CLIP_resnetx4_first500.pth'
        # else:
        #     addr_aug = checkpoint_folder_address + \
        #                'Test_DataAug_AugmentedImageNet10k_80ratio_x2perimg_trainset_CLIP_resnetx4_second500.pth'
        # train_aug_data_half = torch.load(addr_aug)
        # if cls_range[1] < 501:
        #     N_first500=len(train_aug_data_half['label'])


        ## -subtask: load from integrated feature file
        addr_aug_1half = ImageNet10kaug_addr + \
                         'Test_DataAug_AugmentedImageNet10k_80ratio_x2perimg_trainset_CLIP_eval_resnetx4_first5000.pth'
        addr_aug_2half = ImageNet10kaug_addr + \
                         'Test_DataAug_AugmentedImageNet10k_80ratio_x2perimg_trainset_CLIP_eval_resnetx4_second5000.pth'
        ...
        xl_aug = torch.empty((0, d))  # for resnet x4
        yl_aug = []
        ...
        # {'m_train_values': m_train_values,
        #  'std_train_values': std_train_values,
        #  'm_val_values': m_val_values,
        #  'std_val_values': std_val_values
        #  },
        norm_paras=torch.load(norm_base_session_address)
        m_train_values=norm_paras['m_train_values']
        std_train_values=norm_paras['std_train_values']
        ...
        N = len(trainset)
        print('Dimension of labeled data after augmentation:', xl.shape,
              '\nLength of list of labels:', len(yl))
        ...
        super_classes.initialize(method='kmeans++')
        ...
        super_classes.update_supcls_kMeans_raw_RAVE(rawdata, iter=iter_supcls)  # raw data
        ...
        # addr_ini = checkpoint_folder_address + \
        #            'HkPPCAs_{}_run{}_{}supclss_{}-{}cls_{}_PCAq_ini{}_{}shot_lda{:.0e}_ini_KLsupPE.pth'.format(
        #                dataset_name, run_id, num_supcls, cls_range[0] + 1, cls_range[1], self_learner_name,
        #                q_ini, num_labeled_per_class, lda)
        addr_ini = checkpoint_folder_address + \
                   'HkPPCAs_{}_run{}_{}supclss_{}-{}cls_{}_PCAq_ini{}_{}shot_lda{:.0e}_ini_KLsupPE_raw_rave.pth'.format(
                       dataset_name, run_id, num_supcls, cls_range[0] + 1, cls_range[1], self_learner_name,
                       q_ini, num_labeled_per_class, lda)
        ...
        for iter_id in range(num_iters):
            ...
            for batch_id, data in enumerate(train_loader):
                ...
                for obs1_id in range(n_one_batch):
                    ...
                    if len(candid_cls_id) <= max_cls_batch_size:
                        ...
                        torch.cuda.synchronize()
                        t1 = time.time()
                        # score_obs1, _, t3 = modules.score_1obs(x_batch[obs1_id], mu_candid_cls, delta_candid_cls, lda, t)
                        # score_obs1, _, t3 = modules.score_1obs_L_D2(x_batch[obs1_id], mu_candid_cls,
                        ...
                        ith_5k = math.ceil(len(candid_cls_id) / max_cls_batch_size)
                        for i_5k in range(ith_5k):
                            torch.cuda.synchronize()
                            t00=time.time()
                            start_id = i_5k * max_cls_batch_size
                            end_id = min((i_5k + 1) * max_cls_batch_size, len(candid_cls_id))
                            # candid_cls_id_part = candid_cls_id[start_id:end_id]
                            torch.cuda.synchronize()
                            t01=time.time()
                            # mu_candid_cls = mu_online[candid_cls_id_part,:]  # (num_candid_cls, d)
                            #
                            # # print("more than 5000 cand_cls",i_5k,"th",delta[candid_cls_id_part].shape)
                            # # delta_candid_cls_part = delta[candid_cls_id_part]  # .to(device)  # (num_candid_cls, d, d)
                            # # delta_candid_cls_part = delta[candid_cls_id_part]  # .to(device)  # (num_candid_cls, d, d)
                            ...
                            # score_obs1[start_id:end_id], _, t3 = modules.score_1obs_L_D2(x_batch[obs1_id],
                            #                                                              mu_candid_cls,
                            #                                                              L_candid_cls,
                            #                                                              D2_candid_cls, lda, t)
                            # score_obs1[start_id:end_id], _, t3 = modules.score_1obs_L_D2(x_batch[obs1_id],
                ...
                mu_candid_cls = 0; L_candid_cls = 0; D2_candid_cls = 0
                mu_candid_cls_part = 0; L_candid_cls_part = 0; D2_candid_cls_part = 0
                score_obs1=0; score_obs1_mincls=0; score_obs1_minid=0
            ...
            ## - Save parameters in case program crash
            try:
                print(iter_id+1)
                checkpoint_iter_name = 'HkPPCAs_{}_run{}_first{}cls_{}_PCAq{}_{}shot_q{}lda{:.0e}_iter{}.pth'.format(
                    dataset_name, run_id, cls_range[1], self_learner_name, q_ini, num_labeled_per_class, q, lda, iter_id+1
                    )
