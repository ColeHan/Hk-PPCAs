...
class SuperClasses:
    ...
    def initialization(self):
        ...
        self.initial_cls_id = sorted(random.sample(self.all_cls_id, k=self.num_supcls))
        ...
        self.small_jittering = 1e-6 * torch.eye(self.cov_supcls.shape[-1],device=self.device)
        self.cov_inv_supcls = torch.linalg.inv(self.cov_supcls + self.small_jittering)  # TODO: slow, need to improve
        ...
        self.raves = [RAVE() for sk in range(self.num_supcls)]
    ...
    def update_supcls_kMeans(self, iter=10):
        for i in range(iter):
            ...
            for supcls_id in self.all_supcls_id:
                try:
...
def HkPPCAs(num_labeled_per_class, q_ini, q_train, lda, num_iters, dataset_name, self_learner_name, feature_address,
    ...
    for session_id in range(num_sessions):
        ...
        for k in range(num_prestored_cls, num_cls):  # num_prestored_cls=0
            ...
            cov_ini[k] = torch.from_numpy(np.cov(xl[id_labeled, :].T.cpu()))
        ...
        for iter_id in range(num_iters):
            ...
            for batch_id, data in enumerate(train_loader):
                ...
                # mah_dist_to_sup_cls_X = torch.zeros((num_supcls,num_one_batch), device=device)
                candid_supcls_id_X = torch.zeros((num_candid_supcls, num_one_batch), dtype=torch.int)
                for obs1_id in range(num_one_batch):
                    ...
                    for ki, k in enumerate(candid_cls_id):
                        ...
                        try:
                            score_obs1[ki], _ = score(x_batch[obs1_id], mu_online[k], delta_candid_cls_k, lda, t)
                        except:
                            print('ckpt')
            ...
            super_classes = SuperClasses(mu_online, cov_online, nc, num_supcls, device=device)
            super_classes.initialization()
            super_classes.update_supcls_kMeans(iter=iter_supcls)
